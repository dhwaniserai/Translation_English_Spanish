...
class Seq2SeqModel(nn.Module):
    ...
    def forward(
        ...
        encodings, (hidden, cell) = self.encode(source_emb, lengths[0])
    ...
    def encode(
        ...
        encoder_output, (h, c) = self.encoder(packed_source)
    ...
    def decode(
        ...
        Returns:
            ...
            (h, c): the tuple of the final hidden state and cell state from the decoder (per 
                sequence in the batch). `((num_layers, batch_size, hidden_dim), (num_layers, batch_size, hidden_dim))`
        """
        packed_target = nn.utils.rnn.pack_padded_sequence(
            target_embeddings,
            target_lengths,
            batch_first=True,
            enforce_sorted=False
        ...
        decoder_output, (h, c) = self.decoder(packed_target, inits)
        ...
        attention_output = self.attention(decoder_output, encoder_states, padding_mask)
        
        # Concatenate attention output with decoder output
        combined_output = torch.cat([decoder_output, attention_output], dim=-1)
    ...
    def attention(
        ...
        attn_scores = torch.matmul(decoder_states, encoder_states.transpose(1, 2))
        
        # Apply the padding mask if provided
        if padding_mask is not None:
            attn_scores += padding_mask
        
        # Apply softmax along the source sequence length dimension
        attn_weights = torch.nn.functional.softmax(attn_scores, dim=-1)
        
        # Calculate the weighted sum of encoder_states using attention weights
        attn_output = torch.matmul(attn_weights, encoder_states)
